...
def main():
    ...
    ckpt_path = "./results/head_only_vit_small_patch16_dinov3.lvd1689m_b_32_e_10_lr_0.001_sz_224/best_model.pth"   # 训练好的best_model作为起点
    unfreeze_num = 2        # 解冻最后 2 | 4 层
    save_name = f"finetune_final_{unfreeze_num}_best_model.pth"
    batch_size = 32         # 如果显存不够就改小
    num_workers = 4
    num_epochs = 15
    lr_block = 2e-5         # 最后2层blocks: 1e-5 ~ 3e-5（越小越稳）, lr_blocks（8~11）：5e-6 ~ 2e-5（比解冻 2 层更小）
