...
def build_model(model_name: str, num_classes: int, ckpt_path: str = None, device="cuda"):
    """
...
def unfreeze_last_stage_and_head(model: nn.Module, verbose: bool = True):
    """
    timm ConvNeXt 典型结构:
      - model.stem
      - model.stages (ModuleList，长度一般为4: stages[0..3])
      - model.head (分类头)
    这里解冻 stages[3] + head
    """
    freeze_all(model)
    ...
    # 1) 解冻最后一个 stage
    assert hasattr(model, "stages"), "Unexpected timm ConvNeXt structure: no 'stages' attribute"
    last_stage = model.stages[-1]
    for p in last_stage.parameters():
        p.requires_grad = True

    # 2) 解冻 head （不同 timm 版本 head 结构可能稍不同，但一般都有 model.head）
    assert hasattr(model, "head"), "Unexpected timm ConvNeXt structure: no 'head' attribute"
    ...
    return last_stage, model.head


def make_optimizer_stage_head(model: nn.Module, lr_stage=2e-5, lr_head=5e-4, weight_decay=0.01):
    """
    分组学习率：
      - stages.3：小 lr
      - head：大 lr
    ...
    for name, p in model.named_parameters():
        ...
        if name.startswith("stages.3."):
            ...
            # 如果 timm 的命名略有差异，这里兜底：只要 requires_grad=True 但未归类，就给 stage lr
            stage_params.append(p)

    optimizer = torch.optim.AdamW(
        [
            {"params": stage_params, "lr": lr_stage},
...
def main():
    ...
    ckpt_path = "./results/head_only_convnext_tiny.dinov3_lvd1689m_b_32_e_10_lr_0.001_sz_224/best_model.pth"   # 训练好的best_model作为起点
    batch_size = 32         # 如果显存不够就改小
    num_workers = 4
    num_epochs = 20
    lr_stage = 2e-5         # 1e-5 ~ 3e-5（越小越稳）
    lr_head  = 5e-4         # 3e-4 ~ 1e-3，head_only用的 1e-3
    weight_decay = 0.01     # 0.01 起步（你之前 wd=0 对 head-only OK，但解冻 stage 建议加一点）
    ...
    # 模型
    model = build_model(model_name, num_classes, ckpt_path, device)

    # 解冻最后 stage + head
    unfreeze_last_stage_and_head(model, verbose=True)

    # 损失 & 优化器 & 学习率调度
    criterion = nn.CrossEntropyLoss()

    optimizer = make_optimizer_stage_head(
        model,
        lr_stage=lr_stage,
        lr_head=lr_head,
        weight_decay=weight_decay,
    ...
    for epoch in range(num_epochs):
        ...
        stage_lr, head_lr = get_lrs(optimizer)
        print(
            f"Epoch [{epoch+1}/{num_epochs}] "
            f"LR(stage): {stage_lr:.2e} LR(head): {head_lr:.2e} | "
        ...
        if val_acc > best_val_acc:
            ...
            ckpt_path = os.path.join(output_dir, "finetune_best_model.pth")
            torch.save(
                {
